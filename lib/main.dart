...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> _fetchFoodItems() async {
    ...
    final items = await db.fetchItems();
    setState(() {
      _foodItems = items;
      _isLoading = false;
    });
  ...
  void _decreaseQuantity(int foodId) {
    setState(() {
      if (_orderQuantities.containsKey(foodId) &&
          _orderQuantities[foodId]! > 1) {
        ...
        return AlertDialog(
          ...
          content: SizedBox(
            ...
            child: CalendarDatePicker(
              ...
              lastDate: DateTime(2101),   // A reasonable latest date
              onDateChanged: (DateTime date) {
                selectedDate = date; // Update the selected date when user taps
              },
            ),
          ),
          actions: <Widget>[
  ...
  Future<void> _completeOrder() async {
    ...
    if (selectedDate == null) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('A date must be selected'))
      );
      return;
    }
    ...
    try {
      ...
      final newOrderId = await db.insertOrder(orderItems, _orderQuantities, date, _totalPrice);
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text('Your Order (ID: $newOrderId) has been placed successfully. Your Total is \$${_totalPrice.toStringAsFixed(2)}'),
        actions: <Widget>[
          TextButton(
          child: const Text('OK'),
          onPressed: () {
        ...
        children: [
          Container(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(
                'Complete Order (\$${_totalPrice.toStringAsFixed(2)})',
